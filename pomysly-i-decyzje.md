...
- **Sesja 9 (data: 22.05.2026)** — soft delete lig (domknięcie Fazy 1)
  ...
  ### Epizod: fałszywy 404 na /leagues/[id]/edit (NIE był bugiem)
  - Objaw: edycja ligi dawała 404 (przeglądarka), ale terminal pokazywał GET ...200,
    liga pobrana, user OK, żaden notFound/redirect się nie odpalał.
  - Przyczyna: stan dev-servera (zepsuty hot-reload async Server Component) — ten sam
    mechanizm co Navbar w sesji 8. Restart npm run dev to wyleczył. Kod był zdrowy.
  - Potwierdzone na produkcji: po pushu edycja działa, więc to był duch dev-servera.
  - LEKCJA: gdy async Server Component zachowuje się dziwnie → najpierw restart
    npm run dev + twardy refresh, PRZED debugowaniem kodu.
  - LEKCJA (Claude Code): agent dał przekonującą, ale BŁĘDNĄ diagnozę (proxy.ts
    rzekomo ignorowany) — sprzeczną z faktem z terminala (proxy.ts się wykonywał).
    git diff przed commitem ujawnił, że agent zmienił navbar.tsx bez wzmianki.
    NAWYK: git status + git diff po każdej sesji z agentem, nie commitować w ciemno.
  - Przy okazji: kosmetyka Navbara zrobiona (obramowanie nicku, kolor Wyloguj) —
    odhacza dwie pozycje z backloga kosmetycznego.

    - **Sesja 11 (data: 23.05.2026)** — Faza 2: drużyny (teams) ożywione + model awansów

  ### KLUCZOWE ROZRÓŻNIENIE (poprawka błędu z sesji 10)
  - DRUŻYNA = trwała tożsamość gracza w LIDZE (id, nazwa zmienialna, logo w przyszłości).
    Unikalne ID per liga, jeden gracz = jedna drużyna na ligę. NIE zależy od sezonu.
  - ZESPÓŁ/SKŁAD = piłkarze w drużynie w danym SEZONIE, z draftu. Zmienia się co sezon.
    To OSOBNA, przyszła tabela (roster) — drużyna + sezon + piłkarze. Faza 5 (draft).
  - Drużyna może istnieć z pustym składem (jest w lidze, draftu nie było).
  - POPRAWKA: w sesji 10 teams miało błędnie season_id. Przepięte na league_id.

  ### Co zrobione (czysto SQL, ZERO zmian w kodzie aplikacji)
  - teams przepięte: drop + create na nowo z league_id (zamiast season_id).
    unique one_team_per_owner_per_league (league_id, owner_id). RLS SELECT dziedziczy
    widoczność po lidze.
  - Trigger handle_member_activation (after update on league_members): przy
    pending→active tworzy drużynę z team_name. Warunki: old.status is distinct from
    'active' (tylko realna aktywacja, NIE przy zmianie roli), team_name niepuste
    (pomija admina-twórcę), on conflict do nothing (idempotencja + zachowanie ID
    przy powrocie po 'left').
  - Drużyna powstaje przy AKCEPTACJI (Moment 2), nie przy zgłoszeniu — pending to
    jeszcze nie członek. Brak drużyn-sierot po odrzuceniu. joinLeague i approveMember
    nietknięte — drużynę dorabia trigger.

  ### Model AWANSÓW/SPADKÓW — podejście B (DECYZJA, budowa Faza 4+)
  - Jedna liga, wiele POZIOMÓW (tier) wewnątrz. NIE dwie zależne ligi.
  - Awans/spadek = zmiana tier drużyny MIĘDZY sezonami, bez opuszczania ligi
    (drużyna zachowuje trwałe ID). Spójne z modelem drużyna=trwała, sezon=zmienny.
  - tier będzie atrybutem DRUŻYNY-W-SEZONIE (przyszła tabela uczestnictwa/składu),
    nie ligi ani sezonu samego. Dołożymy jako uśpione pole gdy budujemy tę tabelę
    (jak 'finished' w statusie). NIE w sesji 11.
  - Liczba poziomów konfigurowana per liga (1 lub więcej).
  - Ustalanie startowego tier: (a) ręcznie przez admina (świeża liga bez historii),
    (b) wgrywany/import istniejącego podziału, (c) docelowo automatycznie z tabeli
    poprzedniego sezonu (= mechanizm awansu, Faza 4+).
  - WYMAGA wcześniej: meczów i tabel końcowych (Faza 3/4). Awans bez wyników nie ma
    na czym oprzeć decyzji. Najdalsza rzecz w projekcie.

  ### Dług / do zrobienia
  - teams to na razie CICHA warstwa: tworzy się, ale UI dalej pokazuje team_name
    z league_members, nie teams.name. Przepięcie wyświetlania (page.tsx ligi, home)
    + usunięcie team_name z league_members = OSOBNA sesja.
  - Aktywni członkowie sprzed triggera NIE mają drużyn w teams (nie migrujemy wstecz).
  - Admin-twórca nie ma drużyny (brak team_name). Jeśli admin gra — mechanizm
    zakładania drużyny dla admina = do rozważenia osobno.

  ### Lekcja (Claude Code, z poprzedniej sesji — utrwalona)
  - git status + git diff po KAŻDEJ sesji z agentem. Nie commitować w ciemno.
  - Dziwne zachowanie async Server Component → najpierw restart npm run dev, potem debug.

  pomysly-i-decyzje.md
  ...
  - DRUŻYNA = trwała tożsamość gracza w LIDZE (id, nazwa zmienialna, logo w przyszłości).
    Unikalne ID per liga, jeden gracz = jedna drużyna na ligę. NIE zależy od sezonu.
  - ZESPÓŁ/SKŁAD = piłkarze w drużynie w danym SEZONIE, z draftu. Osobna przyszła
    tabela (roster), Faza 5. Drużyna może mieć pusty skład.
  - POPRAWKA: teams w sesji 10 miało błędnie season_id → przepięte na league_id.

  ### Co zrobione
  - teams: drop+create z league_id, unique one_team_per_owner_per_league. RLS SELECT
    dziedziczy po lidze.
  - Trigger handle_member_activation (after update on league_members): przy
    pending→active tworzy drużynę z team_name. Pomija admina-twórcę (brak team_name),
    on conflict do nothing (idempotencja, zachowanie ID przy powrocie po 'left').
  - page.tsx: lista AKTYWNYCH członków czyta nazwę z teams (przez owner_id), fallback
    "Brak drużyny". Panel pending NIETKNIĘTY (czyta league_members.team_name).
  - PODZIAŁ RÓL: league_members.team_name = "proponowana nazwa przy zgłoszeniu" (dla
    pending). teams.name = "oficjalna nazwa drużyny" (dla active). team_name ZOSTAJE.

  ### Model AWANSÓW/SPADKÓW — podejście B (DECYZJA, budowa Faza 4+)
  - Jedna liga, wiele POZIOMÓW (tier) wewnątrz. NIE dwie zależne ligi.
  - Awans/spadek = zmiana tier drużyny między sezonami, bez opuszczania ligi
    (drużyna zachowuje trwałe ID).
  - tier = atrybut drużyny-w-sezonie (przyszła tabela uczestnictwa), dołożymy jako
    uśpione pole gdy budujemy tę tabelę. Liczba poziomów konfigurowana per liga.
  - Startowy tier: (a) ręcznie przez admina (świeża liga), (b) import istniejącego
    podziału, (c) docelowo automat z tabeli poprzedniego sezonu (= awans, Faza 4+).
  - WYMAGA wcześniej meczów i tabel (Faza 3/4).

  ### Decyzje organizacyjne
  - Reset danych przed prawdziwym startem — wszyscy zaczną równo. Nie migrujemy
    starych danych testowych (Opcja A).

  ### Incydent: prywatne notatki w publicznym repo
  - pomysly-i-decyzje.md był commitowany od sesji 9 (5fea512) do PUBLICZNEGO repo —
    zawierał m.in. mail. Usunięty z repo (git rm --cached + .gitignore + push).
    DALEJ w starej historii (5fea512) — do ewentualnego wyczyszczenia.
  - LEKCJA: sprawdzać git check-ignore PRZED założeniem, że plik jest chroniony.

  ### Następne
  - Tworzenie drużyny dla admina-twórcy (dziś nie ma — brak team_name)
  - Tabela uczestnictwa drużyny w sezonie (+ tier) — kolejna sesja Fazy 2
  - players (pula, source_league) → draft (Faza 5)

  - **Sesja 12 (data: 23.05.2026)** — Faza 2: zakładanie własnej drużyny

  ### Co zrobione
  - Server Action createOwnTeam (app/leagues/[id]/team-actions.ts): aktywny członek
    bez drużyny zakłada własną. Walidacja 2-30 znaków (jak joinLeague), unikalność
    nazwy w teams (ilike) ORAZ względem pending team_name (ilike).
  - create-team-button.tsx: przycisk → rozwija input + Utwórz/Anuluj (useTransition).
  - page.tsx: przycisk w bloku "Twoja rola w lidze", widoczny gdy user aktywny
    członek I !teamByOwner.has(user.id).

  ### Reguła widoczności (DECYZJA)
  - Przycisk dla KAŻDEGO aktywnego członka bez drużyny — rola bez znaczenia.
  - Admin-twórca: widzi (nie ma drużyny). Mod: nie (ma drużynę z czasów gracza).
    Super admin GRAJĄCY (dołączył): widzi. Super admin SŁUŻBOWY (nie-członek): nie.
  - Droga B: admin/super NIE musi grać; drużynę zakłada dobrowolnie. Żeby grać
    w lidze, super admin musi najpierw do niej DOŁĄCZYĆ (drużyna należy się członkowi).

  ### Polityka RLS (w bazie, nie w repo)
  - Dodana polityka INSERT na teams: "teams insert own as active member"
    with check: owner_id = auth.uid() AND user jest active członkiem tej ligi.

  ### LEKCJA (ważna)
  - Nowa ścieżka zapisu do tabeli z RLS = nowa polityka. Trigger tworzący drużynę
    omijał RLS (security definer), więc INSERT nie potrzebował polityki. Gdy doszła
    DRUGA droga (user przez akcję) — RLS zablokował ("new row violates row-level
    security policy"), bo brakowało polityki INSERT dla tej ścieżki. Typowy moment,
    gdzie RLS "nagle" blokuje coś, co działało inną drogą.

  ### Następne
  - Tabela uczestnictwa drużyny w sezonie (+ tier, uśpione pod awanse)
  - players (pula, source_league) → draft (Faza 5)

  - **Sesja 13 (data: 23.05.2026)** — Faza 2 Krok A: stany sezonu + zapisy

  ### Co zrobione
  - seasons.status przebudowany: registration / locked / finished (było upcoming/
    active/finished). Default 'registration'. Stara liga active → registration.
  - Trigger handle_new_league: nowy sezon startuje jako 'registration' (było 'active').
  - Polityka RLS UPDATE na seasons: "seasons update by league admin or super"
    (is_league_admin OR is_super_admin), using + with_check. Szeroka (cały wiersz) —
    bramka "tylko status" siedzi w kodzie akcji (wzorzec defense-in-depth).
  - season-actions.ts: closeRegistration (registration→locked) + reopenRegistration
    (locked→registration). Bramka: admin ligi LUB super admin. Tylko zmiana statusu.
  - Przyciski "Zamknij zapisy" / "Otwórz zapisy" w nagłówku ligi przy info o sezonie.
    Widoczne tylko dla admina/super. Zwykły członek widzi tekst "Zapisy zamknięte".

  ### Cykl życia sezonu (DECYZJA, budowane warstwami)
  - Sekwencja: registration (zapisy otwarte) → locked (zamknięte) → [draft] →
    [gra] → finished. Na teraz tylko registration↔locked. Reszta dochodzi z draftem
    i rozgrywkami.
  - Zamknięcie zapisów w Kroku A robi TYLKO przełącznik statusu — NIE tworzy
    uczestników, NIE blokuje dołączania (to Krok B+).
  - Plan dalej: admin podaje daty (draft, start, kolejki) — import lub ręcznie.
    Okno zapisów, admin zamyka kiedy chce, nie później niż start ligi.

  ### Do rewizji w przyszłości (świadomy dług)
  - reopenRegistration bezwarunkowy — OK teraz (locked nic nie pociąga). Gdy
    zamknięcie zacznie tworzyć uczestników/draft → dodać reguły otwarcia
    (np. nie można otworzyć po drafcie, otwarcie kasuje uczestnictwo).
  - currentSeason w page.tsx: .in(['registration','locked']).maybeSingle() zakłada
    JEDEN sezon registration/locked na ligę. Gdy dojdą wielokrotne sezony (np.
    finished + registration) → poprawić, bo maybeSingle może paść na dwóch wierszach.

  ### Następne
  - KROK B: uczestnictwo drużyn w sezonie — lista uczestników krystalizuje się
    przy zamknięciu zapisów (kto był aktywnym członkiem z drużyną do tego momentu).
    Tabela team_id + season_id (+ uśpiony tier pod awanse).
  - Potem: daty/harmonogram, players, draft.

  - Sesja terminarz (data: dzisiejsza) — Punkt 6 z listy 1-10 zrobiony

  ### Co zrobione
  - Tabela matchdays (id, season_id, number, date_from, date_to, deadline, 
    created_at). Unique (season_id, number). RLS: SELECT dziedziczy przez 
    seasons; INSERT/UPDATE/DELETE tylko admin ligi lub super admin. 
    UWAGA: agent zrobił polityki z surowymi joinami do league_members/profiles 
    zamiast helperów is_league_admin/is_super_admin. Działa, ale niespójne 
    ze stylem reszty — do ujednolicenia przy większej refaktoryzacji.
  - closeRegistration rozszerzone o generowanie kolejek: N<2 → 0 kolejek, 
    N parzyste → (N-1)*2, N nieparzyste → N*2. Kolejki bez dat (admin wpisuje).
  - reopenRegistration rozszerzone: kasuje kolejki (spójne z regułą "close 
    tworzy od zera").
  - Server Action updateMatchday (bramka admin/super, walidacja kolejności dat).
  - Komponent matchday-editor.tsx: zwinięty widok z datą, rozwinięty formularz 
    (Od, Do, Deadline UTC).
  - Sekcja "Terminarz" na stronie ligi (widoczna gdy sezon 'locked'). Format 
    daty "10-13 grudnia 2026", deadline mniejszą czcionką, "Termin nieustalony" 
    dla pustych.

  ### Model (utrwalone decyzje)
  - Jedna liga w bazie, dwie ligi (I i II) obsłużone przez tier=1/tier=2 
    w season_participants (podejście B)
  - Terminarz wspólny dla całego sezonu (obu poziomów)
  - Liczba kolejek DYNAMICZNA wg liczby uczestników
  - Kolejki generowane automatycznie przy closeRegistration (bez dat)
  - Admin wpisuje daty ręcznie (import z tabeli terminarza to następny krok, 
    jeśli będzie potrzebny)
  - Jeden deadline typu "piątek 23:59 przed date_from" — na razie admin wpisuje 
    ręcznie, automat może dojść później

  ### Puchar - decyzje na przyszłość
  - Format: 4 grupy po 9 drużyn (36 razem), wspólna tabela pucharowa
  - Top 8 -> 1/8 finału, miejsca 9-24 -> rePPasaże w dwumeczu, 25-36 -> out
  - Losowanie grup: poza stroną, admin wpisuje wynik
  - Harmonogram grup: automat generuje (round-robin z bye dla nieparzystej N)
  - Bezpośredni mecz w tabeli wspólnej: admin rozstrzyga ręcznie gdy drużyny 
    nie grały w grupie
  - Puchar DOPIERO po ukończeniu ligi (bo puchar to nadbudowa: te same 
    kolejki, dwie trójki, wspólny terminarz)

  ### Incydenty naprawione
  - Mail Resend: klucz API wygasł po pół roku, wygenerowany nowy. Problem 
    residualny: onboarding@resend.dev wysyła tylko na główny mail właściciela 
    konta Resend. Do testów: konta tworzone ręcznie w Supabase Auth (bez maila). 
    Docelowe rozwiązanie: zweryfikować własną domenę w Resend przed prawdziwym 
    startem.
  - Hasło głównego admina (sebastian.paterak9@gmail.com) zresetowane SQL-em:
    UPDATE auth.users SET encrypted_password = crypt('...', gen_salt('bf'))
    WHERE email = '...';
  - "Testowa liga" (18.05) nie ma sezonu — powstała przed auto-sezonem 
    (sesja 10). Do usunięcia albo zignorowania. Nie migrujemy wstecz.

  ### Następne z listy 1-10
  - Punkt 7: trójka meczowa (gracz wybiera 3 zawodników z własnego składu 
    na konkretną kolejkę)
  - Punkt 8: automat blokuje trójki po deadline + wpisuje "żelazną trójkę"
  - Punkt 9: wpisywanie wyników (admin/mod wpisuje gole zawodników w kolejce)
  - Punkt 10: tabela ligowa (punkty, gole, bilans, sortowanie 6-kryterialne)
  - Potem: pary w kolejce (kto z kim gra), rozliczenie meczów

  - Sesja trójka meczowa (data: dzisiejsza) — Punkt 7 z listy 1-10 zrobiony

  ### Co zrobione
  - Tabela matchday_lineups (id, matchday_id, season_participant_id, 
    player1_id, player2_id, player3_id, is_iron, created_at, updated_at). 
    player1_id NOT NULL, player2/3 NULLABLE (żeby auto-fill mógł wpisać 
    niepełną trójkę gdy skład nie pozwala na 3 z różnych lig). Constraint 
    distinct_players obsługuje NULL-e. Unique (matchday_id, season_participant_id).
  - RLS: SELECT dziedziczy przez matchdays→seasons→leagues. INSERT/UPDATE — 
    właściciel drużyny (owner_id=auth.uid) LUB mod/admin ligi 
    (is_league_mod_or_admin) LUB super admin. DELETE tylko mod/admin/super. 
    Wszystkie z helperami, nie surowymi joinami.
  - Server Actions w lineup-actions.ts:
    * setLineup — gracz/mod/admin wybiera 3 zawodników. Walidacja: różni, 
      wszyscy ze składu tego uczestnika, 3 różne ligi (case-insensitive), 
      max 2 napastników. Deadline check dla gracza (mod/admin przechodzi 
      zawsze).
    * fillIronLineups — mod/admin klika przycisk, wpisuje żelazne trójki 
      dla kolejek po deadline. Algorytm zachłanny "od lewej" z regułami: 
      iteruje skład po created_at ASC, bierze pierwszego z nowej ligi 
      i (jeśli napastnik) z licznikiem <2. Wpisuje 3 albo mniej jeśli 
      skład nie pozwala. is_iron=true. Zwraca liczbę wpisanych + 
      liczbę niekompletnych.
  - Komponent lineup-editor.tsx: on-change walidacja reguł, wyszarowuje 
    "Zapisz" gdy naruszenie, konkretne komunikaty ("Dwóch zawodników z ligi 
    Bundesliga", "Maksymalnie 2 napastników"). Wyświetlanie: "1. Lewandowski 
    (La Liga) napastnik / 2. — / 3. —" dla niepełnych. Auto-wpisane z is_iron 
    dostają dopisek "(auto)" lub "(auto, niekompletna)" na czerwono.
  - Przycisk fill-iron-lineups-button.tsx w sekcji Terminarz dla mod/admin/super.

  ### Reguły trójki (utrwalone decyzje)
  - Każdy z 3 zawodników z INNEJ ligi (case-insensitive porównanie)
  - Maksymalnie 2 napastników (nigdy 3)
  - Twarda walidacja u gracza — nie może zapisać złej trójki
  - Auto-fill zachłanny "od lewej" — bierze pierwszego z lewej który pasuje
  - Auto-fill wpisuje ile się da (może być 1 lub 2 jeśli skład ubogi)
  - Bez obowiązkowej akceptacji moda — trójka wchodzi od razu, mod nadpisuje 
    tylko jeśli chce
  - Okno wyboru: zawsze przed deadline kolejki
  - Auto-fill uruchamiany RĘCZNIE przez mod/admin (nie automatycznie/cron)

  ### Do zrobienia w przyszłości (świadomy dług)
  - Mechanizm KAR za żelazne trójki (3 OK, 4 = spadek na koniec draftu, 5 = 
    wyrzucenie z ligi) — Poziom C z rekomendacji. Wymaga: liczenia żelaznych 
    per gracz per sezon, mechanizmu draftu (nie ma w apce), UI dla admina 
    do przeglądu żelaznych.
  - Dropdown lig (14 realnych) zamiast pola tekstowego przy zawodniku — 
    wyeliminowałby problem literówek i potrzebę case-insensitive.
  - Rezerwowy (4-ty zawodnik) — regulamin przewiduje, na razie pomijamy.
  - Puchar — druga trójka na kolejkę z pucharem, osobne matchday_lineups 
    dla pucharu. Dojdzie z pucharem.

  ### Następne z listy 1-10
  - Punkt 8: już częściowo w tym kroku (przycisk uzupełniania żelaznych). 
    Docelowo cron/automat po deadline zamiast klikania — dojdzie gdy 
    dodamy service role key lub Vercel Cron.
  - Punkt 9: wpisywanie wyników (admin/mod wpisuje gole zawodników w kolejce)
  - Punkt 10: tabela ligowa (punkty, gole, bilans, sortowanie)

  - Sesja UX składu (data: dzisiejsza) — kompaktowy widok

  ### Co zrobione
  - Punkt 3 z reorganizacji UI: skład podzielony na Napastnicy/Pomocnicy 
    w widoku edycji (roster-management.tsx) i w widoku kompaktowym na stronie ligi
  - Nowy widok kompaktowy w sekcji "Uczestnicy sezonu" (page.tsx):
    format inline "Napastnicy: Kane (Bayern), Douvikas (Como) / Pomocnicy: 
    Gloukh (Ajax)". Bez lig w tym widoku (za dużo miejsca), sam klub.
  - Widoczność: wszyscy członkowie ligi widzą składy wszystkich drużyn
  - Przycisk "Zarządzaj składem" tylko dla admin/mod/super
  - Format edycji (rozwijany przez "Zarządzaj składem") zostaje bez zmian

  ### Decyzje odłożone (Twoja lista reorganizacji UI z tej sesji)
  - Punkt 1 (osobne panele "moje drużyny/składy") — REZYGNACJA, nie robimy
  - Punkt 2 (import terminarza z pliku) — do zrobienia osobno, po ukończeniu 
    funkcjonalności rozgrywki (pary, wyniki, tabela)

    - Sesja reorganizacja UI (data: dzisiejsza) — zakładki na stronie ligi

  ### Co zrobione
  - Strona ligi rozbita na 4 zakładki z URL:
    * /leagues/[id] — Główna (placeholder tabeli ligowej "wkrótce")
    * /leagues/[id]/czlonkowie — członkowie + kompaktowe składy uczestników
    * /leagues/[id]/terminarz — kolejki + trójki meczowe
    * /leagues/[id]/strzelcy — placeholder (wymaga wyników)
  - LeagueHeader jako współdzielony komponent — nagłówek, sezon, przyciski 
    admin/mod (zamknij zapisy, uzupełnij żelazne trójki), panel pending, 
    zakładać drużynę, wyjdź z ligi — replikowany w każdej zakładce
  - TabsNav jako client component z usePathname()
  - Layout.tsx tylko UI ramy (bez zapytań), bo Next.js layouts nie dostają params

  ### Świadome ograniczenia
  - Query wspólne (liga, sezon, uprawnienia) duplikują się per zakładka. 
    Nieodczuwalne dla ligi ~40 wierszy. Do rewizji jeśli kiedyś będzie wolno.
  - revalidatePath w akcjach zmieniony na scope 'layout' żeby zmiany 
    odświeżały cały nagłówek

  ### Punkt 3 z listy reorganizacji (poprzednia sesja)
  - Skład w widoku edycji (roster-management.tsx) — podział 
    Napastnicy/Pomocnicy z nagłówkami
  - Kompaktowy widok składu w zakładce Członkowie — format inline 
    "Napastnicy: X (klub), Y (klub) / Pomocnicy: Z (klub)"

  ### Odłożone/zdecydowane w tej sesji
  - Punkt 1 (osobne panele "moje drużyny") — REZYGNACJA, nie robimy
  - Punkt 2 (import terminarza z pliku) — do zrobienia osobno
  - Tabela ligowa — placeholder w zakładce Głównej, wymaga wyników (punkt 10)
  - Strzelcy — placeholder w zakładce Strzelcy, wymaga wyników (punkt 9)

  - Sesja Design System (data: dzisiejsza) — ujednolicenie UI całej aplikacji

  ### Co zrobione
  - Utworzone komponenty UI w app/components/ui/:
    * Button.tsx (variants: primary/secondary/danger/ghost, sizes: sm/md)
    * Badge.tsx (variants: admin/mod/player/public/private/locked/pairs-ok/neutral)
    * Card.tsx (spójny wrapper z bg-gray-800)
  - Helper lib/utils.ts z cn() (clsx + tailwind-merge)
  - Instalacje: @heroicons/react ^2.2.0, clsx, tailwind-merge
  - Paleta kolorów CSS custom properties w globals.css (referencja, klasy 
    Tailwind zostają w komponentach)
  - Wszystkie emoji w UI zamienione na Heroicons (24/outline):
    * ⚽ (logo Navbar) → ZOSTAJE jako brand identity
    * 👑 → ShieldCheckIcon (admin, super)
    * 🛡️ → ShieldExclamationIcon (mod — ShieldHalfIcon nie istnieje w v2)
    * 🌍 → GlobeAltIcon (publiczna)
    * 🔒 → LockClosedIcon (prywatna, zapisy zamknięte)
    * 🔓 → LockOpenIcon (otwórz zapisy)
    * ✏️ → PencilIcon (edytuj)
    * 🗑️ → TrashIcon (usuń)
    * ⚡ → BoltIcon (uzupełnij żelazne trójki)
    * ⏳ → ClockIcon (oczekujące)
    * 🔄 → ArrowPathIcon (regeneruj)
    * 🚨 → ExclamationTriangleIcon (awaryjny reopen)
    * 🤖 → CpuChipIcon (auto-trójka)
    * ← → ChevronLeftIcon (powrót)
  - 25 istniejących plików przepisane na nowe komponenty (LeagueHeader, 
    wszystkie strony zakładek, wszystkie *-button.tsx, formularze, navbar, 
    home)
  - Segmented button dla napastnik/pomocnik w roster-management.tsx zamiast 
    natywnych radio

  ### Świadome decyzje
  - Modal.tsx pominięty (YAGNI — nigdzie w projekcie nie używamy modala, 
    prompt/confirm robimy przez window.confirm/prompt)
  - Tokeny CSS w globals.css jako REFERENCJA — klasy Tailwind zostają 
    w komponentach (nie zastępujemy Tailwinda)
  - Logo ⚽ w Navbar zostaje (brand identity, nie UI-icon)
  - Emoji w treści tekstowej zostają (np. w nazwach drużyn, w komunikatach 
    narracyjnych)

    - Sesja Wyniki (data: dzisiejsza) — Punkt 9 z listy 1-10 zrobiony

  ### Co zrobione
  - Tabela match_results (id, matchday_id, season_participant_id, 
    roster_player_id, goals, own_goals, created_at, updated_at). 
    UNIQUE (matchday_id, roster_player_id). Constrainty goals/own_goals >= 0.
  - RLS: SELECT dla wszystkich członków ligi (plus publiczne), INSERT/UPDATE/
    DELETE dla mod/admin/super. Defense-in-depth: sprawdzenie że zawodnik 
    jest w matchday_lineups tej kolejki dla danej drużyny.
  - Server Action setMatchResults(matchdayId, results[]) — upsert batch, 
    bramka requireLeagueModOrAdmin, walidacja goals >= 0
  - Komponent match-results-editor.tsx z dwoma trybami:
    * WIDOK (wszyscy): "Kalafaka 4-1 asdasdas" nad parą, strzelcy w trójce 
      z liczbą bramek pogrubioną, samobójcze jako "(N og)" w warning color
    * EDYCJA (mod/admin/super): jeden ekran per kolejka, dla każdej pary 
      formularz z polami gole+og per zawodnik, live-preview wyniku, zapisz 
      wszystko naraz
  - Zmiany w terminarz/page.tsx: fetch match_results, mapa 
    resultsByMatchday, przekazanie do MatchResultsEditor

  ### Reguły wyniku (utrwalone)
  - Wynik drużyny = suma bramek trójki + suma samobójczych zawodników 
    drużyny przeciwnej
  - Samobójczy NIE odejmuje od strzelca, DODAJE do przeciwnika
  - Puste pole = 0 (nic nie strzelił, ale grał)
  - Bez pola "nie zagrał" — kto wystawiony, ten grał
  - Wpisywanie/edycja zawsze możliwe dla mod/admin/super
  - Uprawnienia: admin, mod, super admin (bez zwykłego gracza)
  - Rozróżnienie "wpisane vs nie wpisane" przez obecność rekordów w bazie

  ### Świadome decyzje
  - Samobójcze skrót "og" (nie "sam.") — bo krótsze w wyświetlaniu
  - Layout 108 pól input (dla 18 drużyn) w jednym ekranie — akceptowalne, 
    bo wpisujemy tylko niezerowe wartości, reszta zostaje 0
  - Kolor warning (pomarańcz) dla samobójczych — pierwsze użycie tego 
    koloru w praktyce

    - Sesja Tabela ligowa (data: dzisiejsza) — Punkt 10 z listy 1-10 zrobiony

  ### Co zrobione
  - Kolumny w season_participants: draft_position (integer nullable), 
    manual_position_override (integer nullable)
  - lib/league-standings.ts: pure function calculateStandings, 6-poziomowe 
    sortowanie (Pkt, B+, B- MALEJĄCO, W, opponent pts, draft_position 
    rosnąco). Bez mini-tabeli UEFA — świadoma decyzja dla prostoty.
  - lib/scorers-ranking.ts: ranking strzelców, tylko goals (bez samobójczych)
  - Testy jednostkowe Vitest — 6 scenariuszy pokrywających 6 poziomów 
    sortowania + override
  - Server Actions: setDraftPosition (admin/mod/super), setManualPosition 
    (SUPER ADMIN only)
  - Ważne: setMatchResults automatycznie czyści manual_position_override 
    dla całego sezonu (nowa kolejka najczęściej rozwiąże remis)
  - Zakładka Główna: tabela ligowa osobno dla tier=1 i tier=2, przycisk 
    ✏ dla super admina do nadpisania pozycji
  - Zakładka Strzelcy: ranking strzelców osobno per tier

  ### Świadome ograniczenia (do potencjalnego rozwiązania w przyszłości)
  - Brak mini-tabeli UEFA — jeśli 2+ drużyny idealnie remisują na 
    Pkt+B+, sortuje się wg goli straconych (nietypowa reguła), potem 
    reszta. Super admin ma awaryjne "przesuń w tabeli" na taki edge case.
  - Manual override znika po zapisaniu nowych wyników — świadomie, żeby 
    nie żyć na starych decyzjach

- Seed testowy dodany do "test stworz lige" (data: dzisiejsza)
  - 10 fake kont (testgracz1-10@fake.local, hasła niemożliwe)
  - 10 drużyn "FC Testowa N" + 90 zawodników (6 napastników + 3 pomocników 
    z realnych klubów i lig)
  - Trigger handle_new_user() automatycznie tworzy profiles (nie ręcznie)
  - 22 kolejki (12 uczestników parzyste → (N-1)*2), pary wygenerowane

  ### Odłożone tematy (na następne sesje, ze świeżą głową)
  - Reorganizacja UI: sidebar zamiast poziomych zakładek
  - Nowe zakładki: "Edytuj skład", "Edytuj terminarz" (rozdzielenie od 
    trybu widok), "Transfery", "Puchar Piłkarzyków"
  - Wyświetlanie zawodników po samych nazwiskach (wymaga decyzji: 
    heurystyka last_name albo nowe pole w bazie)
  - Realistyczne nazwy fake drużyn (Warta, Pogoń, itd. zamiast FC Testowa N)
  - Import terminarza z pliku (nadal odłożony)
  - Puchar Piłkarzyków — pełna faza projektowa (grupy, rePPasaże, drabina)
  - Mechanizm kar za żelazne trójki (spadek na koniec draftu, wyrzucenie)
  - Custom domena Resend przed prawdziwym startem
  - Dropdown 14 realnych lig przy dodawaniu zawodnika (zamiast pola tekstowego)

- Sesja Sidebar redesign (data: 09.09.2026)

  ### Co zrobione
  - Zastąpiono poziomy TabsNav pionowym SidebarNav (sidebar-nav.tsx):
    * Desktop: kolumna boczna 208px po lewej, treść po prawej (max-w-5xl, flex-row)
    * Mobile: poziomy scrollowalny pasek u góry (flex-row, overflow-x-auto)
    * Panel moderatora (separator + żółte kolory) widoczny tylko dla admin/mod/super
  - Nowe zakładki w nawigacji:
    * Tabela — /leagues/[id] (była "Główna")
    * Składy — /leagues/[id]/sklady — read-only widok składów dla zalogowanych członków
    * Transfery — /leagues/[id]/transfery — placeholder
    * Puchar Piłkarzyków — /leagues/[id]/puchar — placeholder
    * Żelazne trójki — /leagues/[id]/zelazne-trojki — kto ile razy dostał żelazną
      trójkę w tym sezonie (tabela z licznikiem per drużyna)
    * Edytuj terminarz — /leagues/[id]/edytuj-terminarz — panel moderatora:
      FillIronLineupsButton + PairsManagement + MatchdayEditor + MatchResultsEditor
    * Edytuj skład — /leagues/[id]/edytuj-sklad — panel moderatora:
      DraftPositionEditor + RosterManagement per drużyna
  - Terminarz (/terminarz) oczyszczony z narzędzi moda — tylko widok par, 
    wyniki read-only, LineupEditor dla graczy (własna trójka, przed deadline)
  - Członkowie (/czlonkowie) oczyszczony z sekcji "Uczestnicy sezonu" — 
    tylko lista członków z rolami i MemberRoleControls
  - FillIronLineupsButton przeniesiony z layout.tsx do edytuj-terminarz
  - layout.tsx zmieniony z max-w-3xl na max-w-5xl

  ### Decyzje
  - Składy = osobna zakładka read-only (widok dla wszystkich członków), 
    zarządzanie składem = wyłącznie w "Edytuj skład" (mod only)
  - Panel moderatora wyraźnie oddzielony wizualnie (separator + żółty kolor linków)
  - Mobile: sidebar staje się poziomym menu (jak poprzednie taby) — ta sama UX

- Sesja UI fixes (data: 09.09.2026)

  ### Co zrobione
  - Tabela ligowa: pierwsze 4 miejsca mają subtelne zielone tło (bg-green-900/20), 
    ostatnie 4 czerwone (bg-red-900/20). Przy <8 drużynach zielone ma priorytet.
  - Terminarz: nazwy drużyn powiększone do text-base font-bold (było text-sm font-medium)
  - Terminarz: gracz ze strzeloną bramką wyświetlany białą pogrubioną czcionką
  - Checkbox "mz" (mecz zakończony) w MatchResultsEditor per zawodnik:
    * Zaznaczony + 0 goli → w terminarzu szare imię kursywą
    * Zaznaczony + gol → w terminarzu białe pogrubione bez kursywy
    * Niezaznaczony + 0 goli → szare (bez kursywy, jak dotychczas)
  - Dodana kolumna match_finished boolean NOT NULL DEFAULT false do match_results
  - SQL migracja: ALTER TABLE match_results ADD COLUMN IF NOT EXISTS match_finished boolean NOT NULL DEFAULT false;
